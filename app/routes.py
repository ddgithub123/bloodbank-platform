...
import os
from app import app,myemail,server,mypass
import re, random
from email.mime.text import MIMEText
...
app_login_key='tsvdqiovyhhpulfp'
...
def forget():
    if request.method == 'POST':
        ...
        donor = RDonor.query.filter_by(d_email_id=email).first()
        hospital = RHospital.query.filter_by(h_email_id=email).first()
        user = donor or hospital
...
def register():
    if request.method == 'POST':
        ...
        password = request.form['password']
        admin_type = request.form['admin']  # Donor or Hospital
        ...
        # Create user based on admin_type
        if admin_type == 'donor':
    ...
    if request.method == 'POST':
        ...
        try:
            with SMTP_SSL('smtp.gmail.com', 465) as smtp:
                smtp.login(myemail, app_login_key)
...
#from models import RDonor, RHospital, AdminUser, Donor, BloodDonationRecord, BloodTransfusionRecord, BloodInventory, DonationAppointment, Notification, HospitalNotification
# User Features
#User Logged In
...
def login():
    if request.method == 'POST':
        ...
        print(user_id, password, admin_type)    
        ...
        if admin_type == 'donor':
            ...
            user = AdminUser.query.filter_by(username=user_id).first()

        # Validate user and password
        if user and (admin_type == 'admin' or user.check_password( password) ):
            login_user(user)
            if admin_type == 'admin':
                return redirect(url_for('Admin'))
            else:
